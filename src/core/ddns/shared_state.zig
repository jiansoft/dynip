...
pub fn getProviderSnapshots() [8]ProviderSnapshot {
...
pub fn processProviderRetryCount(provider: DdnsProvider, desired_ip: []const u8) u32 {
    lockProcessProviderStates();
    defer process_provider_mutex.unlock();
...
pub fn memoryWriteProviderAttemptFailure(
    ...
    const retry_count = processProviderRetryCount(provider, desired_ip) + 1;
